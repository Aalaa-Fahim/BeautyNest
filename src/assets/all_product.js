import pro_1 from "./images/pro_1.jpg";
import pro_2 from "./images/pro_2.jpg";
import pro_3 from "./images/pro_3.jpg";
import pro_4 from "./images/pro_4.jpg";
import pro_5 from "./images/pro_5.jpg";
import pro_6 from "./images/pro_6.jpg";
import pro_7 from "./images/pro_7.jpg";
import pro_8 from "./images/pro_8.jpg";
import pro_9 from "./images/pro_9.jpg";
import pro_10 from "./images/pro_10.jpg";
import pro_11 from "./images/pro_11.jpg";
import pro_12 from "./images/pro_12.jpg";
import pro_13 from "./images/pro_13.jpg";
import pro_14 from "./images/pro_14.jpg";
import pro_15 from "./images/pro_15.jpg";
import pro_16 from "./images/pro_16.jpg";
import pro_17 from "./images/pro_17.jpg";
import pro_18 from "./images/pro_18.jpg";
import pro_19 from "./images/pro_19.jpg";
import pro_20 from "./images/pro_20.jpg";
import pro_21 from "./images/pro_21.jpg";
import pro_22 from "./images/pro_22.jpg";
import pro_23 from "./images/pro_23.jpg";
import pro_24 from "./images/pro_24.jpg";
import pro_25 from "./images/pro_25.jpg";
import pro_26 from "./images/pro_26.jpg";
import pro_27 from "./images/pro_27.jpg";
import pro_28 from "./images/pro_28.jpg";
import pro_29 from "./images/pro_29.jpg";
import pro_30 from "./images/pro_30.jpg";
import pro_31 from "./images/pro_31.jpg";
import pro_32 from "./images/pro_32.jpg";
import pro_33 from "./images/pro_33.jpg";
import pro_34 from "./images/pro_34.jpg";
import pro_35 from "./images/pro_35.jpg";
import pro_36 from "./images/pro_36.jpg";
import pro_37 from "./images/pro_37.jpg";
import pro_41 from "./images/pro_41.jpg";
import pro_43 from "./images/pro_43.jpg";
import pro_47 from "./images/pro_47.jpg";
import pro_48 from "./images/pro_48.jpg";
import pro_49 from "./images/pro_49.jpg";
import pro_50 from "./images/pro_50.jpg";
import pro_51 from "./images/pro_51.jpg";
import pro_53 from "./images/pro_53.jpg";
import pro_54 from "./images/pro_54.jpg";
import pro_56 from "./images/pro_56.jpg";
import pro_57 from "./images/pro_57.jpg";
import pro_58 from "./images/pro_58.jpg";


let all_data_product = [
  {
    id: 1,
    name: "Organic face foundation from cosmatic organic care",
    category: "beauty",
    image: pro_1,
    new_price: 50.0,
    old_price: 80.5,
    description: "This organic face foundation provides flawless coverage with a natural finish. Made with organic ingredients, it's gentle on the skin, ensuring a radiant look all day long.",
  },
  {
    id: 2,
    name: "Lip Glow Oil color reviver-cherry oil ",
    category: "beauty",
    image: pro_2,
    new_price: 90.0,
    old_price: 120.5,
    description: "Infused with cherry oil, this lip glow oil nourishes and revives the natural color of your lips. Perfect for adding a glossy, hydrated look to your makeup routine.",
  },
  {
    id: 3,
    name: "BOBBL Brown Face foundation long-wear even fashion SPF 15",
    category: "beauty",
    image: pro_3,
    new_price: 60.0,
    old_price: 100.5,
    description: "Achieve a smooth and even complexion with BOBBL Brown's long-wear face foundation. With SPF 15 protection, it's ideal for all-day wear and perfect for any fashion look.",
  },
  {
    id: 4,
    name: "MAYBELLINE instant age rewind eraser multi-use concealer ... NON COMEDOGGENIC",
    category: "beauty",
    image: pro_4,
    new_price: 100.0,
    old_price: 150.0,
    description: "This multi-use concealer from Maybelline is a game-changer! Instantly erases dark circles, blemishes, and fine lines without clogging pores. Suitable for sensitive skin.",
  },
  {
    id: 5,
    name: "RubyRose Delinearidor Black falsies lash lift ... Mascara waterproof",
    category: "beauty",
    image: pro_5,
    new_price: 99.0,
    old_price: 122.5,
    description: "Get the perfect lash lift with RubyRose's waterproof mascara. Designed to give your lashes volume and length while withstanding humidity and water.",
  },
  {
    id: 6,
    name: "benefit bene tint rose tinte, lip&cheek stain",
    category: "beauty",
    image: pro_6,
    new_price: 88.0,
    old_price: 105.5,
    description: "A cult favorite! This rose-tinted stain from Benefit is perfect for adding a natural flush to your lips and cheeks, lasting all day for a fresh look.",
  },
  {
    id: 7,
    name: "CARVAO DETOX mask preta peel-off",
    category: "beauty",
    image: pro_7,
    new_price: 43.0,
    old_price: 55.5,
    description: "Deep clean your pores with the CARVAO Detox peel-off mask. This black mask is designed to remove impurities, leaving your skin smooth and rejuvenated.",
  },
  {
    id: 8,
    name: "L'OREAL PARIS revitallift filler+Hyaluronic acid ... [Hyaluronic acid + caffeine] eye serum",
    category: "beauty",
    image: pro_8,
    new_price: 97.0,
    old_price: 135.5,
    description: "Revitalize tired eyes with L'Oreal's Revitallift filler serum. Formulated with hyaluronic acid and caffeine, this serum reduces puffiness and hydrates the delicate under-eye area.",
  },
  {
    id: 9,
    name: "SEHGLAM color bloom lyquid blush",
    category: "beauty",
    image: pro_9,
    new_price: 32.5,
    old_price: 45.5,
    description: "Add a pop of color to your cheeks with SEHGLAM's Color Bloom liquid blush. Lightweight and easy to blend, it's perfect for achieving a natural, glowing look.",
  },
  {
    id: 10,
    name: "NARS liqued ace foundation long-wear even fashion SPF 15",
    category: "beauty",
    image: pro_10,
    new_price: 83.0,
    old_price: 117.0,
    description: "NARS brings you a long-lasting foundation with SPF 15. This lightweight formula offers full coverage while protecting your skin from sun damage.",
  },
  {
    id: 11,
    name: "Four-colored pinkey beauty blinder, soft",
    category: "beauty",
    image: pro_11,
    new_price: 13.0,
    old_price: 25.5,
    description: "Achieve flawless makeup application with this four-colored soft beauty blender. Perfect for blending foundation, concealer, and more for an airbrushed finish.",
  },
  {
    id: 12,
    name: "DIOR Black falsies lash lift ... Mascara waterproof",
    category: "beauty",
    image: pro_12,
    new_price: 81.0,
    old_price: 96.5,
    description: "DIOR's waterproof mascara offers dramatic volume and a lash-lifting effect, perfect for all-day wear, even in wet conditions.",
  },
  {
    id: 13,
    name: "BORB THIS WAY multi-use sculpting concealer ... super coverage",
    category: "beauty",
    image: pro_13,
    new_price: 75.0,
    old_price: 125.0,
    description: "Get super coverage with BORB's multi-use sculpting concealer. Ideal for contouring and concealing, it provides a flawless, long-lasting finish.",
  },
  {
    id: 14,
    name: "LANEIGE lip sleeping mask [BERRY] 20g pink",
    category: "beauty",
    image: pro_14,
    new_price: 47.0,
    old_price: 52.0,
    description: "Wake up to soft, hydrated lips with LANEIGE's lip sleeping mask. Infused with berry extracts, this overnight treatment nourishes and repairs lips as you sleep.",
  },
  {
    id: 15,
    name: "Violet multi-purpose set of brushes with container",
    category: "beauty",
    image: pro_15,
    new_price: 55.0,
    old_price: 65.0,
    description: "This versatile brush set is perfect for all your beauty needs. With a variety of brushes for face and eyes, it comes in a stylish violet container for easy storage.",
  },
  {
    id: 16,
    name: "Four Fance colored beauty brushes ultra soft.",
    category: "beauty",
    image: pro_16,
    new_price: 59.0,
    old_price: 78.5,
    description: "These ultra-soft, fancy-colored brushes are perfect for applying makeup with precision and ease. Ideal for both beginners and professionals.",
  },
  {
    id: 17,
    name: "COSRX advanced snail 92 all in one cream.",
    category: "facial",
    image: pro_17,
    new_price: 39.0,
    old_price: 49.0,
    description: "Hydrate and repair your skin with COSRX's advanced snail 92 all-in-one cream. Formulated with snail mucin, it provides intense moisture and improves skin texture.",
  },
  {
    id: 18,
    name: "MIDO face serum with Hyaluronic acid .. simon curian, M.D, Beverly Hills.",
    category: "facial",
    image: pro_18,
    new_price: 59.0,
    old_price: 64.5,
    description: "This hydrating face serum by MIDO, enriched with hyaluronic acid, locks in moisture and enhances skin elasticity. Trusted by experts from Beverly Hills.",
  },
  {
    id: 19,
    name: "Electric face steamer",
    category: "facial",
    image: pro_19,
    new_price: 69.5,
    old_price: 75.0,
    description: "Open up your pores and rejuvenate your skin with this electric face steamer. Perfect for deep cleansing and relaxing facial treatments at home.",
  },
  {
    id: 20,
    name: "RODIAl VIT C brightening mask .. face mask",
    category: "facial",
    image: pro_20,
    new_price: 85.0,
    old_price: 120.5,
    description: "Brighten and revitalize your skin with RODIAL's Vitamin C mask. This powerful mask targets dark spots and uneven skin tone, leaving your complexion radiant and refreshed.",
  },
  {
    id: 21,
    name: "BIOAQUA Aloe vera 92% soothing gel",
    category: "facial",
    image: pro_21,
    new_price: 17.0,
    old_price: 23.5,
    description: "Soothe and hydrate your skin with BIOAQUA's Aloe Vera soothing gel. This 92% natural formula calms irritated skin and provides instant relief from dryness.",
  },
  {
    id: 22,
    name: "CeraVe hydrating facial cleanser for normal to dry skin",
    category: "facial",
    image: pro_22,
    new_price: 70.0,
    old_price: 72.5,
    description: "A gentle cleanser that hydrates and helps restore the protective skin barrier, perfect for normal to dry skin.",
  },
  {
    id: 23,
    name: "FOREO rechargable facial masage brush",
    category: "facial",
    image: pro_23,
    new_price: 59.0,
    old_price: 107.5,
     description: "A silicone facial brush that gently exfoliates and enhances your skincare routine with a soothing massage.",
  },
  {
    id: 24,
    name: "iShine GoLd face care golden collection, Full-set ",
    category: "facial",
    image: pro_24,
    new_price: 129.5,
    old_price: 145.5,
    description: "A luxurious full set that includes premium face care products for a radiant and youthful glow.",
  },
  {
    id: 25,
    name: "AVEDA revival purple toning .. tones, cools, and neutralize",
    category: "facial",
    image: pro_25,
    new_price: 78.0,
    old_price: 83.5,
    description: "A purple toning treatment that neutralizes unwanted brassiness in blonde and silver hair.",
  },
  {
    id: 26,
    name: "GOMMA pure LE corps with rose oil body scrub",
    category: "facial",
    image: pro_26,
    new_price: 22.0,
    old_price: 29.5,
    description: "A nourishing body scrub with rose oil that exfoliates and hydrates, leaving your skin soft and smooth.",
  },
  {
    id: 27,
    name: "COSRX Advanced snail 96 Mucin Power Essence",
    category: "facial",
    image: pro_27,
    new_price: 99.0,
    old_price: 109.5,
    description: "A powerful essence enriched with snail mucin that promotes skin repair, hydration, and elasticity.",
  },
  {
    id: 28,
    name: "CeraVe Renewing SA cleanser for normal skin",
    category: "facial",
    image: pro_28,
    new_price: 85.0,
    old_price: 120.5,
     description: "A renewing cleanser that exfoliates and removes dirt while helping to restore the skin barrier.",
  },
  {
    id: 29,
    name: "LA ROCHE-POSAY Vitamin C serum",
    category: "facial",
    image: pro_29,
    new_price: 81.0,
    old_price: 87.5,
    description: "A brightening vitamin C serum that reduces the appearance of fine lines and promotes a radiant complexion.",
  },
  {
    id: 30,
    name: "The Ordinary Glycolic Acid 7% toning solution",
    category: "facial",
    image: pro_30,
    new_price: 109.0,
    old_price: 115.5,
    description: "An exfoliating toning solution that improves skin texture and radiance while minimizing pores.",
  },
  {
    id: 31,
    name: "COSRX AHA/BHA clarifying treatment toner",
    category: "facial",
    image: pro_31,
    new_price: 79.0,
    old_price: 97.5,
    description: "A clarifying toner that helps control oil and prevent breakouts while maintaining moisture.",
  },
  {
    id: 32,
    name: "GARNIeR SkinActive Vitamin C* Anti-dark spot serum 3.5%",
    category: "facial",
    image: pro_32,
    new_price: 48.0,
    old_price: 53.5,
     description: "A brightening serum that targets dark spots and uneven skin tone for a more luminous complexion.",
  },
  {
    id: 33,
    name: "Florance dreamy dew moistrurizer hydratant rose de reve",
    category: "facial",
    image: pro_33,
    new_price: 89.0,
    old_price: 125.5,
    description: "A lightweight moisturizer with rose extract that hydrates and leaves your skin feeling refreshed.",
  },
  {
    id: 34,
    name: "TRESemme expert 7 hair shamppo, conditioner, and hair mask collection",
    category: "hair",
    image: pro_34,
    new_price: 43.0,
    old_price: 58.5,
     description: "A complete hair care collection that nourishes, strengthens, and revitalizes your hair.",
  },
  {
    id: 35,
    name: "MIELLE Rosemary mint scalp & hair oil 59 ml",
    category: "hair",
    image: pro_35,
    new_price: 40.0,
    old_price: 42.5,
    description: "A scalp and hair oil infused with rosemary and mint that promotes hair growth and healthy scalp.",
  },
  {
    id: 36,
    name: "Wooden comb natural 2 pices small size",
    category: "hair",
    image: pro_36,
    new_price: 27.0,
    old_price: 33.0,
    description: "A natural wooden comb set designed to detangle and style hair without causing damage.",
  },
  {
    id: 37,
    name: "L'OREAL PARIS ELVITAL dream lenght shampoo and hair condtioner",
    category: "hair",
    image: pro_37,
    new_price: 22.0,
    old_price: 37.0,
    description: "A nourishing shampoo and conditioner duo that helps achieve long, healthy-looking hair.",
  },
  {
    id: 41,
    name: "REVLON spinner electric brush",
    category: "hair",
    image: pro_41,
    new_price: 199.0,
    old_price: 259.5,
    description: "An innovative electric brush that effortlessly styles and smoothens hair for a polished look.",
  },
  {
    id: 43,
    name: "TRESemme Keratin smooth hair collection",
    category: "hair",
    image: pro_43,
    new_price: 85.0,
    old_price: 120.5,
    description: "A keratin-infused hair care collection that smoothens frizz and leaves hair soft and manageable.",
  },
  {
    id: 47,
    name: "Johanson's baby lotion nourishes babys skin for 24 hours 444 ml",
    category: "baby",
    image: pro_47,
    new_price: 39.0,
    old_price: 45.5,
    description: "A gentle lotion formulated to nourish and moisturize your baby's sensitive skin for all-day hydration.",
  },
  {
    id: 48,
    name: "Aveeno baby calming comfort bath with natural oat extract",
    category: "baby",
    image: pro_48,
    new_price: 85.0,
    old_price: 120.5,
    description: "A soothing bath treatment that calms and comforts your baby's delicate skin with natural oat extract.",
  },
  {
    id: 49,
    name: "Johanson's baby oil nourishes babys skin for 24 hours",
    category: "baby",
    image: pro_49,
    new_price: 19.0,
    old_price: 26.5,
    description: "A moisturizing baby oil that provides gentle nourishment and hydration for your baby's skin.",
  },
  {
    id: 50,
    name: "mustela NEWBORN Arrival Gift Set",
    category: "baby",
    image: pro_50,
    new_price: 95.0,
    old_price: 110.5,
    description: "A special gift set designed for newborns, featuring gentle products that nurture delicate skin.",
  },
  {
    id: 51,
    name: "Johanson's baby collection nourishes babys skin for 24 hours full set",
    category: "baby",
    image: pro_51,
    new_price: 99.0,
    old_price: 137.5,
    description: "A complete baby care collection that nourishes and protects your baby's skin for all-day hydration."
  },
  {
    id: 53,
    name: "baby Dove Baby massage oil , rich moisture",
    category: "baby",
    image: pro_53,
    new_price: 31.0,
    old_price: 35.5,
    description: "A rich moisturizing oil that provides gentle massage and hydration for your baby's delicate skin.",
  },
  {
    id: 54,
    name: "Shea Moisture BABY-moisture wash & shampoo",
    category: "baby",
    image: pro_54,
    new_price: 87.0,
    old_price: 95.5,
    description: "A 2-in-1 wash and shampoo that gently cleanses and moisturizes your baby's hair and skin."
  },
  {
    id: 56,
    name: "Everoeden kids multi-vitamin face wash",
    category: "baby",
    image: pro_56,
    new_price: 125.0,
    old_price: 131.5,
    description: "A gentle face wash packed with vitamins that cleanses and nourishes your child's delicate skin."
  },
  {
    id: 57,
    name: "Baby Dove night time lotion 384 ml",
    category: "baby",
    image: pro_57,
    new_price: 39.0,
    old_price: 48.5,
  },
  {
    id: 58,
    name: "ATTITUDE natural care, Babt mineral sunscreeen 30 SPF - F.P.s",
    category: "baby",
    image: pro_58,
    new_price: 85.0,
    old_price: 120.5,
  },
];

export default all_data_product;
